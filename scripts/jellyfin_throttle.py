...
def get_active_remote_streams():
    ...
    try:
        ...
        for session in sessions:
            ...
            if "NowPlayingItem" in session and not session.get("PlayState", {}).get("IsPaused", True):
                remote_endpoint = session.get("RemoteEndPoint")
                if remote_endpoint and is_remote_ip(remote_endpoint):
                    remote_streams += 1
                    logger.debug(f"Remote stream detected: {session.get('UserName')} on {session.get('Client')} from {remote_endpoint}")
        ...
        if remote_streams != -1:
            ...
            if should_throttle != last_state:
                try:
                    ...
                    if should_throttle:
                        logger.info(f"Active remote streams: {remote_streams}. Activating alternative rate limits.")
                        qbt_client.transfer_set_speed_limits_mode(True)
                    else:
                        logger.info("No active remote streams. Activating global rate limits.")
