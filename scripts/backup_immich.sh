#!/bin/bash

# Define variables
START_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")"/.. && pwd)"
RESTIC_PASSWORD="/home/harryzhong/restic_password"
BACKUP_SOURCE="/media/storage/immich_library"
BACKUP_REPO_REMOTE="rclone:gdrive:Backups/immich_library"
KEEP_OPTIONS="--keep-daily 7 --keep-weekly 4 --keep-monthly 6"
LOG_FILE="$START_DIR/logs/immich_backup.log"
MAX_LOG_SIZE=$((5 * 1024 * 1024))

rotate_log() {
  timestamp=$(date +"%Y%m%d_%H%M%S")
  mv "$LOG_FILE" "${LOG_FILE}.${timestamp}"
  echo "[$(date)] Log rotated: ${LOG_FILE}.${timestamp}"
}

if [[ -f "$LOG_FILE" && $(stat -c%s "$LOG_FILE") -gt $MAX_LOG_SIZE ]]; then
  rotate_log
fi

echo "[$(date)] Starting Immich library backup" >> "$LOG_FILE"

(
  echo "============================================================"
  echo "Immich Library Backup started"
  echo "============================================================"

  # Backup to remote repo (excluding generated cache/transcode directories)
  echo "Backing up to remote repo..."
  restic -p $RESTIC_PASSWORD -r $BACKUP_REPO_REMOTE --verbose backup $BACKUP_SOURCE \
    --exclude "$BACKUP_SOURCE/thumbs" \
    --exclude "$BACKUP_SOURCE/encoded-video"

  # Prune remote repo
  echo "Pruning remote repo..."
  restic -p $RESTIC_PASSWORD -r $BACKUP_REPO_REMOTE --verbose forget $KEEP_OPTIONS --prune --cleanup-cache

  echo "Immich library backup complete"

) 2>&1 | ts "%Y-%m-%d %H:%M:%S" | tee -a $LOG_FILE
