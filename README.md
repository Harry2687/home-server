...
Rclone is used alongside restic to enable cloud connectivity for off-site backups. Application data backups are scheduled weekly using `./scripts/backup_appdata.sh`. Immich library files are backed up `./scripts/backup_immich.sh`.
